...
class Puzzle(object):
    ...
    def astarsearch(self):
        ...
        frontier = collections.deque([Node(self.einit_state,self.egoal_state,0,"N", None)]) #add the initial state
        seen = set()
        ctr = 0
        while frontier:
            frontier = collections.deque(sorted(list(frontier), key= lambda node: node.fscore())) #sorts the queue 
            ctr+= 1
            
            ##self.debugQueue(frontier,ctr)

            tempNode = frontier.popleft()
